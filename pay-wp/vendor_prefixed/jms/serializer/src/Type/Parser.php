...
final class Parser implements ParserInterface
{
    ...
    private function visit()
    {
        ...
        throw new SyntaxError(sprintf('Syntax error, unexpected "%s" (%s)', $this->lexer->token->value, $this->getConstant($this->lexer->token->type)));
    }
    /**
     * @return TypeArray
     */
    ...
    private function match(int $token): void
    {
        if (!$this->lexer->lookahead) {
            throw new SyntaxError(sprintf('Syntax error, unexpected end of stream, expected %s', $this->getConstant($token)));
        }
        if ($this->lexer->lookahead->type === $token) {
            $this->lexer->moveNext();
            return;
        }
        throw new SyntaxError(sprintf('Syntax error, unexpected "%s" (%s), expected was %s', $this->lexer->lookahead->value, $this->getConstant($this->lexer->lookahead->type), $this->getConstant($token)));
    }
