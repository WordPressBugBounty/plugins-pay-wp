...
final class UnionHandler implements SubscribingHandlerInterface
{
    ...
    public function serializeUnion(SerializationVisitorInterface $visitor, mixed $data, array $type, SerializationContext $context): mixed
    {
        if ($this->isPrimitiveType(gettype($data))) {
            $resolvedType = ['name' => gettype($data), 'params' => []];
        } else {
    ...
    public function deserializeUnion(DeserializationVisitorInterface $visitor, mixed $data, array $type, DeserializationContext $context): mixed
    {
        ...
        if (array_filter($type['params'][0], static fn(array $type): bool => $type['name'] === $dataType || isset(self::$aliases[$dataType]) && $type['name'] === self::$aliases[$dataType])) {
            return $context->getNavigator()->accept($data, ['name' => $dataType, 'params' => []]);
        }
    ...
    {
        return in_array($type, ['int', 'integer', 'float', 'double', 'bool', 'boolean', 'true', 'false', 'string', 'array'], \true);
    }
