    ...
    protected function loadMetadataFromFile(ReflectionClass $class, string $file): ?BaseClassMetadata
    {
        ...
        if (array_key_exists('virtual_properties', $config)) {
            foreach ($config['virtual_properties'] as $methodName => $propertySettings) {
                if (isset($propertySettings['exp'])) {
                    $virtualPropertyMetadata = new ExpressionPropertyMetadata($name, $methodName, $this->parseExpression($propertySettings['exp']));
        ...
        if (!$excludeAll) {
            ...
            foreach ($propertiesMetadata as $propertyKey => $pMetadata) {
                $isExclude = \false;
                $isExpose = $pMetadata instanceof VirtualPropertyMetadata || $pMetadata instanceof ExpressionPropertyMetadata || isset($propertiesData[$propertyKey]);
                ...
                if (!empty($pConfig)) {
                    ...
                    if (isset($pConfig['union_discriminator'])) {
                        $pMetadata->setType(['name' => 'union', 'params' => [null, $pConfig['union_discriminator']['field'], $pConfig['union_discriminator']['map']]]);
                    }
                }
