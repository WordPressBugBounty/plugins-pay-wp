...
class TypedPropertiesDriver implements DriverInterface
{
    ...
    {
        return ['int', 'float', 'bool', 'boolean', 'true', 'false', 'string', 'double', 'iterable', 'resource'];
    }
    /**
        ...
        foreach ($classMetadata->propertyMetadata as $propertyMetadata) {
            ...
            try {
                ...
                if ($this->shouldTypeHint($reflectionType)) {
                    ...
                    $propertyMetadata->setType(['name' => 'union', 'params' => [$this->reorderTypes(array_map(fn(string $type) => $this->typeParser->parse($type), array_filter($reflectionType->getTypes(), [$this, 'shouldTypeHintInsideUnion'])))]]);
                }
