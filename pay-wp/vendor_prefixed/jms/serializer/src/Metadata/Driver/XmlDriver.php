    ...
    protected function loadMetadataFromFile(\ReflectionClass $class, string $path): ?BaseClassMetadata
    {
        ...
        foreach ($elem->xpath('./virtual-property') as $method) {
            if (isset($method->attributes()->expression)) {
                $virtualPropertyMetadata = new ExpressionPropertyMetadata($name, (string) $method->attributes()->name, $this->parseExpression((string) $method->attributes()->expression));
            } else {
        ...
        if (!$excludeAll) {
            ...
            foreach ($propertiesMetadata as $propertyKey => $pMetadata) {
                $isExclude = \false;
                $isExpose = $pMetadata instanceof VirtualPropertyMetadata || $pMetadata instanceof ExpressionPropertyMetadata || isset($propertiesNodes[$propertyKey]);
                ...
                if (!empty($pElem)) {
                    ...
                    if (isset($pElem->{'union-discriminator'})) {
                        ...
                        $pMetadata->setType(['name' => 'union', 'params' => [null, (string) $colConfig->attributes()->field, $map]]);
                    }
